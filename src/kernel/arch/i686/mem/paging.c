...
void initialize_paging(unsigned int memsize) {
  ...
  set_frame(0);
  for (void *idx = (void *)0x1000; idx < placement_address + 0x3000;
       idx += 0x1000)
#else
  ...
  for (void *idx = placement_address + 0x3000; idx < heap_start;
       idx += 0x1000) {
  ...
  kernel_heap =
      create_heap((void *)KERNEL_HEAP_START, (void *)KERNEL_HEAP_END, 0, 0);
