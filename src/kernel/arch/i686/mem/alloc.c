...
heap_t *create_heap(void *start, void *hend, unsigned char supervisor, unsigned char ro)
{
  //assert(!start&0xFFF);
  //assert(!end&0xFFF);
...
void *khalloc(size_t size, unsigned int align, heap_t*heap)
{
  ...
  if(head_ptr == NULL|| head_ptr->is_free == 0 || head_ptr->size < full_size)
  {
  ...
  heap_header_t * new_head = (heap_header_t *)((unsigned int)head_ptr + head_ptr->size - full_size);
  head_ptr->size -= full_size;
  new_head->size = full_size;
  new_head->is_free = 0;
  new_head->next = NULL;
  new_head->prev = head_ptr->prev;
  heap_footer_t *new_foot = (heap_footer_t *)((unsigned int)new_head + full_size - sizeof(heap_footer_t));
  new_foot->size = full_size;

  heap_footer_t * head_foot = (heap_footer_t *)((unsigned int)head_ptr + head_ptr->size - sizeof(heap_footer_t));
...
{
  heap_header_t *head_ptr = (heap_header_t *)((unsigned int)p - sizeof(heap_header_t));
  ...
  if(head_ptr->is_free == 0)
  {
    ...
    {
      heap_footer_t *next_foot = (heap_footer_t *)((unsigned int)next_ptr + next_ptr->size - sizeof(heap_footer_t));
...
heap_header_t *prev_contig_block(heap_header_t *block)
{
  heap_footer_t * prev_foot = (heap_footer_t *)((unsigned int)block - sizeof(heap_footer_t));
