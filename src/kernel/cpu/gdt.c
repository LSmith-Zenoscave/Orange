...
void initialize_gdt() {
  ...
  fill_gdt_entry(0, (gdt_t){.base = 0, .limit = 0, .type = 0},
                 false);  // Null Segment
  fill_gdt_entry(1, (gdt_t){.base = 0, .limit = 0xFFFFFFFF, .type = 0x9A},
                 false);  // Code Segment
  fill_gdt_entry(2, (gdt_t){.base = 0, .limit = 0xFFFFFFFF, .type = 0x92},
                 false);  // Data Segment
  fill_gdt_entry(3, (gdt_t){.base = 0, .limit = 0xFFFFFFFF, .type = 0xFA},
                 false);  // User-Mode Code Segment
  fill_gdt_entry(4, (gdt_t){.base = 0, .limit = 0xFFFFFFFF, .type = 0xF2},
                 false);  // User-Mode Data segment
 ...
 * ---------------------------------------------------------------------------
 */
void fill_gdt_entry(unsigned int index, gdt_t source, bool is_tss) {
  ...
  gdt.entries[index].granularity |= (unsigned char)((source.limit >> 16) & 0x0F);

  // Set Base
  gdt.entries[index].base_low =
      (unsigned short)((unsigned int)source.base & 0xFFFF);
  gdt.entries[index].base_middle =
      (unsigned char)(((unsigned int)source.base >> 16) & 0xFF);
  gdt.entries[index].base_high =
      (unsigned char)(((unsigned int)source.base >> 24) & 0xFF);
 ...
 * ---------------------------------------------------------------------------
 */
static void write_tss(unsigned int index, unsigned short ss0,
                      unsigned int esp0) {
  ...
  fill_gdt_entry(
      index, (gdt_t){.base = base, .limit = (unsigned int)limit, .type = 0xE9},
      true);
